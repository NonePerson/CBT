...
namespace CBT
{
    class Program
    {
        ...
        static void Main()
        {
            ...
            Console.WriteLine();
            Console.WriteLine("Press M/m to add 'meta-cognition' to your thoughts or 'intuation' about your feelings in a situation");
            Console.WriteLine("(thoughts about your thoughts or feeling that arise when you are reminded of feelings from the situation)");
            Console.WriteLine();
        ...
        private static void ChoosingMetaCognition(string[] situationText, int i)
        {
            ...
            Console.WriteLine($"Situation name: {situationText[1]}");
            Console.WriteLine();
            Console.WriteLine("Do you want to enter thoughts/s about your thought/s, or intuations about your feeling/s?");
            Console.WriteLine("(Press T/t to enter thought/s about thought/s, or F/f to enter intuations about feeling/s)");
            Console.WriteLine();
            Console.WriteLine("Tip: Writing intuations about your feelings beforehand is recommended,");
            Console.WriteLine("Because those can, by themselves, remind you of the thoughts you may've had in the situation.");
            Console.WriteLine();
        ...
        private static void WritingMetaCognition(string[] situationText, string[] allInfo, int infoAmount, string forFilling, int situationNum)
        {
            ...
            for (int i = 1; i <= infoAmount; i++)
            {
                ...
                string[] textPreEdit;
                if(forFilling == "feeling")
                {
                    Console.WriteLine($"Press C/c to skip entering intuations about the above {forFilling},");
                    Console.WriteLine($"Or enter your intuations/s about the above {forFilling}:");
                }
                else
                {
                ...
                if (input.ToUpper() != "C")
                {
                    ...
                    if (forFilling == "feeling")
                    {
                        inputForMetaCognition[0 + ((i - 1) * 8)] = $"an intuation about the above {forFilling}:";
                        inputForMetaCognition[4 + ((i - 1) * 8)] = "Notes about this intuation:";
                    }
                    else
                    {
                    ...
                    {
                        Console.WriteLine($"Do you want to add another intuation about this {forFilling}? (Press Y/y or N/n)");
                    }
                    Console.WriteLine();
        ...
        private static void AddingEvenMoreMetaCognition(string[] situationText, int countingLines, string[] allInfo, XElement relevantMetaCount, int i, string[] inputForMetaCognition, string forFilling, SituationElements elements, XElement CurrectSituation, int situationNum, XElement path)
        {
            ...
            if (forFilling == "feeling")
            {
                Console.WriteLine($"Press C/c to skip entering intuation/s about the above {forFilling},");
                Console.WriteLine($"Or enter your intuation/s about the above {forFilling}:");
            }
            else
            {
                Console.WriteLine($"Press C/c to skip entering {forFilling}s about the above {forFilling},");
        ...
        private static void ViewingInfo(string[] BasicSituation, string[] allInfo, int infoAmount, string ForFilling, int situationNum)
        {
            ...
            for (int i = 1; i <= infoAmount; i++)
            {
                        ...
                        if (File.Exists($@"{BasicSituation[1]}{ForFilling}{z}meta.txt"))
                        {
                            ...
                            for (int c = 0; c < checkedMetaFile.Count(); c++)
                            {
                                if (checkedMetaFile[c].Contains($@"a meta-cognition ({ForFilling}) about the above {ForFilling}:")
                                    || checkedMetaFile[c].Contains($@"an intuation about the above {ForFilling}:"))
                                {
                                    if (c == 0)
                                    {
                ...
                if (ForFilling != "behaviour" && int.Parse(relevantMetaCount.Value) > 0 && allMetaForThis.Count > 0)
                {
                    ...
                    {
                        Console.WriteLine();
                        Console.WriteLine($"Press T/t to view intuations/s about this {ForFilling}");
                    }
                    else
                    {
                        Console.WriteLine();
        ...
        private static void ViewingMetaCognitionsForEachInfo(string[] BasicSituation, int countingLines, string[] allInfo, List<string[]> allMetaForThis, string ForFilling)
        {
            ...
            for (int m = 0; m < allMetaForThis.Count; m++)
            {
                ...
                for (int line = 0; line < allMetaForThis[m].Count(); line++)
                {
                    if (ForFilling == "feeling")
                    {
                        if (allMetaForThis[m].ElementAt(line) == $@"an intuation about the above {ForFilling}:")
                        {
                            writing = line;
                        }
                    }
                ...
                {
                    Console.WriteLine($"Press N/n to go read the next intuation about the current {ForFilling} for this situation,");
                    Console.WriteLine($"Or P/p to go read the previous intuation about the current {ForFilling} for this situation.");
                }
                Console.WriteLine();
                ...
                else if (metaInput.ToUpper() == "N")
                {
                    if (m == allMetaForThis.Count() - 1)
                    {
                        ...
                        {
                            Console.WriteLine($"There are no more intuations about the current {ForFilling}.");
                        }
                        Console.WriteLine();
                ...
                else if (metaInput.ToUpper() == "P")
                {
                    if (m == 0)
                    {
                        ...
                        if (ForFilling == "thought")
                        {
                            Console.WriteLine($"There are no previous {ForFilling}s about the current {ForFilling}.");
                        }
                        else if (ForFilling == "feeling")
                        {
                            Console.WriteLine($"There are no previous intuations about the current {ForFilling}.");
                        }
                        Console.WriteLine();
                        Console.WriteLine($"Press any key to return to viewing your next {ForFilling}/s in the situation.");
                        Console.WriteLine();
